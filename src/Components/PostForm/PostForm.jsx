...
function PostForm() {
  ...
  return (
    <div className='flex justify-center p-x mt-25'>
      ...
      <dialog ref={dialogRef} id="edit_profile_modal" className='w-full max-w-xl rounded-[2rem] border border-slate-200 bg-white p-0 text-slate-900 shadow-[0_35px_100px_rgba(15,23,42,0.24)] outline-none'>
        <div className='flex flex-col gap-5 p-6'>
          {/*//! this is for profile logo*/}

          <div className="flex items-center gap-4">
            <img src={ProfileImage} alt="profile image" className="rounded-full h-16 w-16 object-cover ring-4 ring-sky-100" />
            {/* <input type = "file" ref={ProfileRef} onChange={handleProfileImage} hidden /> */}
            <div>
              <p className='text-xs font-semibold uppercase tracking-[0.2em] text-slate-400'>Compose</p>
              <h3 className='text-xl font-bold text-slate-900'>Share a new tweet</h3>
            </div>
          ...
          <form className='flex flex-col gap-4 ' onSubmit={handleSubmitTweetForm} >

            <div>
              <textarea type="text" placeholder='Write something worth stopping for...' value={content} onChange={(e) => setContent(e.target.value)} className='soft-input tweet-textarea' />
            ...
            <div className='flex items-center justify-between gap-3 rounded-3xl bg-slate-50 px-3 py-3'>
              {/*//! tweet image ko pass krne ke lie dialog box me */}
              {/* <input type = "file" ref={TweetImageRef}  hidden/> */}
              <button type="button" onClick={() => TweetImageRef.current.click()} className='rounded-full border border-sky-100 bg-white p-3 text-sky-600 hover:border-sky-200 hover:bg-sky-50 cursor-pointer'> <FaImages /></button>

              <div className='flex items-center gap-2'>
                <button
                  type="submit"
                  disabled={uploading}
                  className={`${uploading ? 'bg-sky-300 cursor-not-allowed' : 'bg-sky-500 hover:bg-sky-600 cursor-pointer'} text-white px-5 py-3 rounded-full font-semibold`}
                ...
                <button type="button" onClick={() => dispatch(closeDialog())} className='bg-white hover:bg-slate-100 text-slate-700 px-5 py-3 rounded-full text-sm font-semibold cursor-pointer border border-slate-200'>Close</button>
              </div>
